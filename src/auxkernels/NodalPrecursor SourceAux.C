/*#include "NodalPrecursorSourceAux.h"

registerMooseObject("pertApp", NodalPrecursorSourceAux);

InputParameters
NodalPrecursorSourceAux::validParams()
{
...
NodalPrecursorSourceAux::NodalPrecursorSourceAux(const InputParameters & parameters)
...
RealEigenVector
NodalPrecursorSourceAux::computeValue()
{
  return - ((*_chi_nu_sigma_f)[_qp] * _flux[_qp]).sum() * 
  ( (*_delayed_fraction)[_qp].cwiseQuotient( (*_decay_constants)[_qp]) );
}
*/