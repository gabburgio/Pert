#include "UOSphMaterial.h"
#include "ArrayElementIntegralUserObject.h"

registerMooseObject("pertApp",UoSphMaterial);

InputParameters
UoSphMaterial::validParams()
{
    InputParameters params = Material::validParams();
    
    params.addRequiredParam<UserObjectName>("integrator_uo", "___");  
...
UoSphMaterial::UoSphMaterial(const InputParameters & parameters) :
    Material(parameters),
    ...
    _integrator(const_cast<ArrayElementIntegralUserObject&>(getUserObject<ArrayElementIntegralUserObject>("integrator_uo"))),
...
void
UoSphMaterial::computeQpProperties()
{
    ...
    RealEigenVector integrals = _integrator.getValue();

    sph_factors.conservativeResize(_v_diffusivity.size());

    for(int i=0; i< _v_diffusivity.size(); i++ ) 
    {
    sph_factors(i)= _ref_phi_mg(i)/integrals(i);
    }
