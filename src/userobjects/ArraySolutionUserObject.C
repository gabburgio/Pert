...
#include "ArraySolutionUserObject.h"
...
registerMooseObject("MooseApp", ArraySolutionUserObject);

InputParameters
ArraySolutionUserObject::validParams()
{
  ...
  params.addParam<MultiMooseEnum>(
      ...
      "form p = R1*(R0*x*m - t)/s.  Then the values provided by the ArraySolutionUserObject at point x "
...
Threads::spin_mutex ArraySolutionUserObject::_solution_user_object_mutex;

ArraySolutionUserObject::ArraySolutionUserObject(const InputParameters & parameters)
...
ArraySolutionUserObject::~ArraySolutionUserObject() {}

void
ArraySolutionUserObject::readXda()
{
...
void
ArraySolutionUserObject::readExodusII()
{
  // Define a default system name
  if (_system_name == "")
    _system_name = "ArraySolutionUserObjectSystem";
  ...
  if (num_exo_times == 0)
    mooseError("In ArraySolutionUserObject, exodus file contains no timesteps.");
  ...
  else
  {
    if (_exodus_time_index > num_exo_times)
      mooseError("In ArraySolutionUserObject, timestep = ",
                 _exodus_time_index,
                 ", but there are only ",
                 num_exo_times,
                 " time steps.");
...
Real
ArraySolutionUserObject::directValue(const Node * node, const std::string & var_name) const
{
...
Real
ArraySolutionUserObject::directValue(const Elem * elem, const std::string & var_name) const
{
  // Get the libmesh variable and system numbers
  unsigned int var_num = _system->variable_number(var_name);
  unsigned int sys_num = _system->number();
...
void
ArraySolutionUserObject::initialize()
{
}

void
ArraySolutionUserObject::finalize()
{
}

void
ArraySolutionUserObject::timestepSetup()
{
...
void
ArraySolutionUserObject::execute()
{
}

void
ArraySolutionUserObject::initialSetup()
{
  ...
  // Several aspects of ArraySolutionUserObject won't work with a DistributedMesh:
  ...
  // Produce an error for an unknown file type
    ...
    mooseError("In ArraySolutionUserObject, invalid file type: only .xda, .xdr, .exo and .e supported");
...
MooseEnum
ArraySolutionUserObject::getSolutionFileType() const
{
  return _file_type;
}

void
ArraySolutionUserObject::updateExodusTimeInterpolation(Real time)
{
...
bool
ArraySolutionUserObject::updateExodusBracketingTimeIndices(Real time)
{
  if (_file_type != 1)
    mooseError(
        "In ArraySolutionUserObject, getTimeInterpolationData only applicable for exodusII file type");
...
unsigned int
ArraySolutionUserObject::getLocalVarIndex(const std::string & var_name) const
{
...
Real
ArraySolutionUserObject::pointValueWrapper(Real t,
                                      const Point & p,
                                      ...
                                      const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  mooseError(
      "ArraySolutionUserObject::pointValueWrapper reaches line that it should not be able to reach.");
  return 0.0;
}

Real
ArraySolutionUserObject::pointValue(Real t,
                               const Point & p,
                               const std::string & var_name,
                               const std::set<subdomain_id_type> * subdomain_ids) const
{
...
Real
ArraySolutionUserObject::pointValue(Real libmesh_dbg_var(t),
                               const Point & p,
...
ArraySolutionUserObject::discontinuousPointValue(Real t,
                                            const Point & p,
                                            const std::string & var_name,
                                            const std::set<subdomain_id_type> * subdomain_ids) const
{
...
std::map<const Elem *, Real>
ArraySolutionUserObject::discontinuousPointValue(Real libmesh_dbg_var(t),
                                            Point pt,
                                            const unsigned int local_var_index,
                                            const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  if (_file_type == 1 && _interpolate_times)
  {
    ...
    if (map.size() != map2.size())
      mooseError("In ArraySolutionUserObject::discontinuousPointValue map and map2 have different size");
    ...
    for (auto & k : map)
    {
      if (map2.find(k.first) == map2.end())
        mooseError(
            "In ArraySolutionUserObject::discontinuousPointValue map and map2 have differing keys");
      Real val = k.second;
...
RealGradient
ArraySolutionUserObject::pointValueGradientWrapper(
    Real t,
    const Point & p,
    const std::string & var_name,
    const MooseEnum & weighting_type,
    const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  mooseError("ArraySolutionUserObject::pointValueGradientWrapper reaches line that it should not be "
             "able to reach.");
  return RealGradient(0.0, 0.0, 0.0);
}

RealGradient
ArraySolutionUserObject::pointValueGradient(Real t,
                                       const Point & p,
                                       const std::string & var_name,
                                       const std::set<subdomain_id_type> * subdomain_ids) const
{
...
RealGradient
ArraySolutionUserObject::pointValueGradient(Real libmesh_dbg_var(t),
                                       Point pt,
                                       const unsigned int local_var_index,
                                       const std::set<subdomain_id_type> * subdomain_ids) const
{
...
ArraySolutionUserObject::discontinuousPointValueGradient(
    Real t,
    const Point & p,
    const std::string & var_name,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
Real
ArraySolutionUserObject::directValue(dof_id_type dof_index) const
{
...
Real
ArraySolutionUserObject::evalMeshFunction(const Point & p,
                                     const unsigned int local_var_index,
...
std::map<const Elem *, Real>
ArraySolutionUserObject::evalMultiValuedMeshFunction(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  for (auto & k : temporary_output)
  {
    mooseAssert(k.second.size() > local_var_index,
                "In ArraySolutionUserObject::evalMultiValuedMeshFunction variable with local_var_index "
...
RealGradient
ArraySolutionUserObject::evalMeshFunctionGradient(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
std::map<const Elem *, RealGradient>
ArraySolutionUserObject::evalMultiValuedMeshFunctionGradient(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
ArraySolutionUserObject::variableNames() const
{
  return _system_variables;
}

bool
ArraySolutionUserObject::isVariableNodal(const std::string & var_name) const
{
...
Real
ArraySolutionUserObject::scalarValue(Real /*t*/, const std::string & var_name) const
{
  unsigned int var_num = _system->variable_number(var_name);
...
void
ArraySolutionUserObject::readBlockIdMapFromExodusII()
{
